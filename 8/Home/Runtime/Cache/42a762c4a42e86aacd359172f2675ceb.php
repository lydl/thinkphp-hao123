...
<TABLE cellSpacing=1 cellPadding=5 width="100%" border=0><!--无内容--><!--无内容--><!--无内容--><!--无内容--><!--无内容--><!--无内容--><!--无内容--><!--无内容--><!--无内容-->
  <TBODY>
   	<?php if(is_array($listdata)): $k = 0; $__LIST__ = $listdata;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$child): $mod = ($k % 2 );++$k; if(in_array(($k), explode(',',"1,3,5,7,9,11,13,15,17"))): ?><TR bgColor="<?php if(in_array(($k), explode(',',"1,5,9,13,17,21,25,29"))): ?>#eff7f0<?php else: ?>#e5f2e5<?php endif; ?>">
